...
class MIDIGenerator:
    ...
    def __init__(self, params: Optional[MusicParameters] = None):
        ...
        self.midi.addTempo(self.params.track, 0, self.params.tempo)
    ...
    def generate_from_path(
        ...
        for i in range(min(path_length, 32)):  # Max 32 notes
            ...
            self.midi.addNote(
                self.params.track, self.params.channel, pitch, time, duration, velocity
            )
    ...
    def _add_chord(
        self, root: int, time: float, duration: float, velocity: int
    ) -> None:
        """Add a triad chord.
